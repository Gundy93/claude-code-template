...
- **§12.7 분류기 동작 보강**: 첫 요청의 워크스페이스 컨텍스트(CLAUDE.md·git 상태)만으로 분류기가 발화할 수 있다는 점(`claude --safe-mode`로 원인 분리), **Opus 5의 생물학 범주는 폴백 없이 거절**(사이버만 Opus 4.8로 재실행)이라는 비대칭, `/config`로 자동 전환을 끄는 법, 범주 기반 폴백의 v2.1.219+ 요건.
