...
> 사용법: 새 프로젝트 디렉토리에서 Claude Code의 Opus 5 / high 세션을 열고, 아래 모든 내용을 첫 메시지로 그대로 붙여넣는다.
...
- 모델별 비용 비율(2026년 7월 기준): Haiku 4.5 ($1/$5) : Sonnet 5 ($3/$15, 2026-08-31까지 인트로 $2/$10) : Opus 5 ($5/$25, 4.8과 동일). Opus 5·Sonnet 5는 기본 effort high, 적응형 추론 기본 ON(4.7 이후 세대는 새 토크나이저로 같은 텍스트가 약 30% 더 많은 토큰이 된다). Opus 위의 Fable 5($10/$50)는 이 경량 셋업과 무관하다(default 아님 — 핸드북 §5.4).
...
이 작업은 Opus 5 기본값인 **high**로 진행한다. 표준 부트스트랩보다 작업량이 적어 xhigh는 불필요. 단, 파일 생성·검증은 충실히 수행.
