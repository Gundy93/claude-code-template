...
> 사용법: 새 프로젝트 디렉토리에서 Claude Code의 Opus 5.5 세션을 **effort high로 열고**(`claude --effort high` — 세션은 기본 medium으로 시작한다), 아래 모든 내용을 첫 메시지로 그대로 붙여넣는다.
...
- 모델별 비용 비율(2026년 9월 기준): Haiku 4.5 ($1/$5) : Sonnet 5 ($2/$10) : Opus 5.5 ($4/$20). Opus 5.5는 기본 effort medium·thinking 상시 on, Sonnet 5는 기본 effort high·적응형 추론 기본 ON(4.7 이후 세대는 새 토크나이저로 같은 텍스트가 약 30% 더 많은 토큰이 된다). Opus 위의 Fable 5.1($10/$50)은 이 경량 셋업과 무관하다(default 아님 — 핸드북 §5.4).
...
이 작업은 effort **high**를 전제로 한다(Opus 5.5 기본값 medium보다 위). 표준 부트스트랩보다 작업량이 적어 xhigh는 불필요. 단, 파일 생성·검증은 충실히 수행.
