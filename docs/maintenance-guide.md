...
> **이 가이드는 v0.2.0 기준이다(사이클 1 반영).** 트리거 분류·영향 파일은 첫 실제 사이클(Opus 4.8)을 거쳐 보완됐다. cadence·임계값은 아직 표본이 1회뿐이라 잠정값이며, 추가 모델 사이클을 거친 뒤 재조정한다(§7 참조). (v0.3.0은 모델 사이클이 아닌 콘텐츠 사이클이라 cadence 표본은 1회로 유지.)
