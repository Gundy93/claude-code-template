...
> **이 가이드의 트리거·영향 매핑·cadence는 모델 사이클 1회(v0.2.0, Opus 4.8) 표본 기준이다.** 트리거 분류·영향 파일은 그 첫 실제 사이클을 거쳐 보완됐다. cadence·임계값은 아직 표본이 1회뿐이라 잠정값이며, 추가 모델 사이클을 거친 뒤 재조정한다(§7 참조). (v0.3.0은 모델 사이클이 아닌 콘텐츠 사이클이라 cadence 표본은 1회로 유지.)
