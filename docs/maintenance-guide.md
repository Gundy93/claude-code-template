...
> **이 가이드의 트리거·영향 매핑·cadence는 모델 사이클 2회(v0.2.0 Opus 4.8, v0.4.0 Sonnet 5) 표본 기준이다.** 트리거 분류·영향 파일은 그 실제 사이클들을 거쳐 보완됐다. cadence·임계값은 이제 표본이 2회이며 두 사이클 모두 major "4주 이내" 창을 지켰다 — 다만 둘 다 수동 실행이라 분기 cadence 가설은 여전히 미검증이다(§7 참조). (v0.3.0은 모델 사이클이 아닌 콘텐츠 사이클이라 cadence 표본에서 제외.)

## 0. 핵심 통찰 — 무엇이 자동으로 따라오고 무엇이 안 따라오는가

에이전트 frontmatter의 `model: opus / sonnet / haiku`는 **family alias**다. Claude Code가 현재 디폴트(Opus 4.8, Sonnet 5, Haiku 4.5; 2026년 7월 기준)로 자동 해석한다. **신모델이 나와도 `model` 별칭만으로는 에이전트 파일을 거의 건드릴 필요가 없다** — 단, 신기능을 채택하면(예: 아래 T5의 `effort` frontmatter 필드) 해당 에이전트 파일은 손대게 된다.
...
**cadence는 아직 잠정값**이다(사이클 1회 표본). 반복 운영(다음 모델 사이클)을 거친 뒤 재조정 — §7 참조.
...
### 다음 모델 사이클에서 검토할 것
- §2 cadence 임계값이 반복 운영에서도 맞는가? (표본 2 — 두 번 다 "4주 이내" 준수. 분기 cadence는 여전히 미검증.)
